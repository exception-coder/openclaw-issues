# Capabilities Index

## Abilities

- [自动记录 OpenClaw 自身问题并推送 issues 仓库](./abilities/auto-record-and-push-openclaw-issues.md)
- [通过 skill 驱动 OpenClaw 自维护与知识沉淀](./abilities/skill-driven-openclaw-self-maintenance-and-knowledge-curation.md)
- [区分 systemEvent 提醒展示与真实会话/投递故障并完成排查](./abilities/diagnose-systemevent-reminder-vs-session-delivery-failures.md)
- [修复 installed build 中的会话 `heartbeat` 误标与 `public` skills 发现缺失](./abilities/repair-installed-build-session-heartbeat-and-public-skill-discovery.md)

## Scenarios

- [2026-03-12 · 利用 skill + issues 仓库实现“修复即归档、归档即推送”工作流](./scenarios/2026-03-12-openclaw-self-maintenance-workflow.md)

## 使用原则

- `abilities/` 记录稳定能力
- `scenarios/` 记录真实落地案例
- 一个能力可以对应多个案例
- 一个案例也可以反过来证明某项能力已经成熟
